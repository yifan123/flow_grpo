...
def pipeline_with_logprob(
    ...
    ref: Optional[PipelineImageInput] = None,
    ...
    for i in range(batch_size):
        ...
        negative_prompt_embeds_mask_list.append(negative_prompt_embeds_mask)
    ...
    negative_prompt_embeds_mask = torch.cat(negative_prompt_embeds_mask_list, dim=0)
    
    if image is not None and not (isinstance(image, torch.Tensor) and image.size(1) == self.latent_channels):
        image = self.image_processor.resize(image, calculated_height, calculated_width)
        prompt_image = self.image_processor.preprocess(image, calculated_height, calculated_width)
        image = prompt_image.unsqueeze(2)
    if ref is not None and not (isinstance(ref, torch.Tensor) and ref.size(1) == self.latent_channels):
        ref = self.image_processor.resize(ref, calculated_height, calculated_width)
        ref_image = self.image_processor.preprocess(ref, calculated_height, calculated_width)
        ref = ref_image.unsqueeze(2)
