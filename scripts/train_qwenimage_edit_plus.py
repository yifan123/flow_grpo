...
from flow_grpo.diffusers_patch.qwenimage_edit_plus_pipeline_with_logprob import pipeline_with_logprob
...
# class bGenevalPromptImageDataset(Dataset):
...
class GenevalPromptImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Assuming 'image' in metadata contains a path to the image file
        # image_path = self.metadatas[idx]['image']
        ctrl_image_path = self.metadatas[idx]['ctrl_image']
        ref_image_path = self.metadatas[idx]['ref_image']
        sup_image_path = self.metadatas[idx]['sup_image']
        item["prompt_with_image_path"] = f"{self.prompts[idx]}_{ctrl_image_path}"
        ctrl_image = Image.open(os.path.join(self.dataset, ctrl_image_path)).convert('RGB')
        item["ctrl_image"] = ctrl_image
        ref_image = Image.open(os.path.join(self.dataset, ref_image_path)).convert('RGB')
        item["ref_image"] = ref_image
        sup_image = Image.open(os.path.join(self.dataset, sup_image_path)).convert('RGB')
        item["sup_image"] = sup_image
        return item
    ...
    def collate_fn(examples):
        prompts = [example["prompt"] for example in examples]
        metadatas = [example["metadata"] for example in examples]
        ctrl_images = [example["ctrl_image"] for example in examples]
        ref_images = [example["ref_image"] for example in examples]
        sup_images = [example["sup_image"] for example in examples]
        prompt_with_image_paths = [example["prompt_with_image_path"] for example in examples]
        return prompts, metadatas, ctrl_images, ref_images, sup_images, prompt_with_image_paths
...
def eval(pipeline, test_dataloader, config, rank, local_rank, world_size, device, global_step, reward_fn, executor, autocast, ema, transformer_trainable_parameters):
    ...
    for test_batch in tqdm(
        ...
        prompts, prompt_metadata, ctrl_images, ref_images, sup_images, _ = test_batch
        ctrl_images = [ctrl_image.resize((config.resolution, config.resolution)) for ctrl_image in ctrl_images]
        ref_images = [ref_image.resize((config.resolution, config.resolution)) for ref_image in ref_images]
        sup_images = [sup_image.resize((config.resolution, config.resolution)) for sup_image in sup_images]
        with autocast():
            with torch.no_grad():
...
def main(_):
    ...
    if rank == 0:
        wandb.init(
            project="flow_grpo_qwenimage_edit_plus",
            # mode="disabled"
        )
    logger.info(f"\n{config}")
    ...
    while True:
        ...
        for i in tqdm(
            ...
            prompts, prompt_metadata, ctrl_images, ref_images, prompt_with_image_paths = next(train_iter)
            ctrl_images = [ctrl_image.resize((config.resolution, config.resolution)) for ctrl_image in ctrl_images]
            ref_images = [ref_image.resize((config.resolution, config.resolution)) for ref_image in ref_images]
            sup_images = [sup_image.resize((config.resolution, config.resolution)) for sup_image in sup_images]
