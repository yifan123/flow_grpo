...
def main(_):
    ...
    if is_deepspeed_zero3_enabled():
        ...
        reward_fn = getattr(flow_grpo.rewards, 'multi_score')(accelerator.device, config.reward_fn)
        eval_reward_fn = reward_fn#getattr(flow_grpo.rewards, 'multi_score')(accelerator.device, config.reward_fn)
    ...
    while True:
        ...
        if config.train.beta>0:
            if (reset==True) or (epoch - prev_epoch)>15:
                with torch.no_grad():
        ...
        if config.per_prompt_stat_tracking:
            ...
            for item in ['aesthetic', 'imagereward', config.main_reward]:   
                _, indices = torch.sort(torch.tensor(gathered_rewards[item]), descending=True)  
            ...
            reward_std=reward_std[-1]-(reward_std[0]+reward_std[1])/2
