...
def main(_):
    ...
    pipeline.transformer.set_adapter("old")
    ...
    while True:
        ...
        if config.train.beta>0:
            if (epoch !=0 and reset==True) or (epoch - prev_epoch)>config.reset_freq:
                with torch.no_grad():
                    ...
                    # set_old = True
                reset = False
                # if epoch - prev_epoch > config.reset_freq:
                #     thres = max(thres - 1e-5, 1e-5)
                # if epoch - prev_epoch < config.reset_freq // 2:
                #     thres = min(thres+1e-5,1e-4)
                prev_epoch = epoch
        samples["rewards"]["ori_avg"] = samples["rewards"][config.main_reward]
        ...
        samples["rewards"]["avg"] = samples["rewards"][config.main_reward].unsqueeze(1).repeat(1, num_train_timesteps)
        # gather rewards across processes
        gathered_rewards = {key: accelerator.gather(value) for key, value in samples["rewards"].items()}
        gathered_rewards = {key: value.cpu().numpy() for key, value in gathered_rewards.items()}
        gathered_dists = accelerator.gather(samples["avg_dist"]).cpu()
        ...
        if config.per_prompt_stat_tracking:
            ...
            # import pdb;pdb.set_trace()
            advantages = stat_tracker.update(prompts, gathered_rewards['avg'],np.nan_to_num(gathered_dists), type='gardo')
            reward_std = []
            for item in ['aesthetic', 'imagereward', config.main_reward]:
                _, indices = torch.sort(torch.tensor(gathered_rewards[item]), descending=True)  # 降序排序获取索引
                ranks = torch.empty_like(torch.tensor(gathered_rewards[item])).long()
                ranks[indices] = torch.arange(1, len(gathered_rewards[item])+1)
                reward_std.append(ranks.float())
            # import pdb;pdb.set_trace()
            # reward_std=torch.std(torch.stack(reward_std),dim=0)
            reward_std=reward_std[-1]-(reward_std[0]+reward_std[1])/2    #uncertainty estimation
            dynamic_controller.update(np.array([p for p in reward_std.cpu().numpy() if p>=0]))
            threshold = np.percentile(np.array([p for p in reward_std.cpu().numpy() if p>=0]), 100 - dynamic_controller.beta*100)
            reward_std = reward_std > threshold
                ...
                wandb.log(
                    {
                        ...
                        'kl_perc':dynamic_controller.beta,
                    },
                    step=global_step,
                )
            stat_tracker.clear()
        ...
        samples["reward_std"] = (
            reward_std.reshape(accelerator.num_processes, -1)[accelerator.process_index]
            .to(accelerator.device)
        )
        if accelerator.is_local_main_process:
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                        ...
                        policy_loss = (std_dev_t[:,0,0,0]/torch.mean(std_dev_t[:,0,0,0]))*torch.maximum(unclipped_loss, clipped_loss)
                        kl_loss = torch.zeros_like(policy_loss).float()
                        if config.train.beta > 0:
                            if True in idx:
                                kl_loss = torch.mean((torch.square(prev_sample_mean[idx]-prev_sample_mean_ref[idx])) 
                                           / (2 * std_dev_t[idx] ** 2))
                               
                                loss = torch.mean(policy_loss) + config.train.beta*torch.mean(kl_loss)
                            else:
                        ...
                        if config.train.beta > 0:
                            info["kl_loss"].append(torch.mean(kl_loss))
                    ...
                    if accelerator.sync_gradients:
                        ...
                        if info["kl_loss"] > thres:
                            reset=True
                        info.update({"epoch": epoch, "inner_epoch": inner_epoch,"grad_norm":grad_norm.detach().item()})
